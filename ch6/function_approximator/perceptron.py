...
class SLP(torch.nn.Module):
    ...
    def __init__(self, input_shape, output_shape, device=torch.device("cuda:0")):
        ...
        self.linear1 = torch.nn.Linear(self.input_shape, self.hidden_shape, device=self.device)
        self.out = torch.nn.Linear(self.hidden_shape, output_shape, device=self.device)
